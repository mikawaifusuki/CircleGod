...
export default function EditComponentPage({
    ...
    const fetchComponent = async () => {
      try {
        const response = await fetch(
          `/api/dashboard/${params.id}/components/${params.componentId}`
        );
        if (!response.ok) {
  ...
  }, [params.id, params.componentId, status]);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-8">Edit Component</h1>
      ...
      <form onSubmit={handleSubmit} className="max-w-lg">
        ...
        <div className="mb-4">
          ...
          <textarea
            id="config"
            name="config"
            defaultValue={JSON.stringify(component.config, null, 2)}
            required
            rows={6}
